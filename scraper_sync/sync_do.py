...
def fill_missing_timeslots(room_schedule):
    ...
    for slot in room_schedule:
        ...
        if slot["timeslot"] == target_timeslot_array[0]:  # already exists
            ...
            new_schedule.append(
                slot
            )  # can afford to be coded this way due to the nature of sorted arrays
            del target_timeslot_array[
                0
            ]  # can afford to be coded this way due to the nature of sorted arrays
    return new_schedule
    ...
    try:
        ...
        try:
            ...
            if not frame:
                ...
                while True:
                    current_date_value = frame.query_selector(
                    ...
                    if current_date_value == DATE_FORMATTED:
                        ...
                        frame.query_selector(
                            "a#BtnDpcNext.btn"
                        ).click()  # click the button until desired date, which by default is the next day
                ...
                select_start_time_input = frame.query_selector(
                    "select#TimeFrom_c1_ctl04"
                )  # options tags can then be selected by value, values range from 00:00 to 23:30
                ...
                select_end_time_input = frame.query_selector_all(
                ...
                select_capacity_input = frame.query_selector(
                ...
                if EQUIPMENT_ARRAY:
                    ...
                    if frame.is_visible("#DropMultiEquipmentList_c1_textItem"):
                        frame.click(
                            "#DropMultiEquipmentList_c1_textItem"
                        )  # opens the dropdown list
                ...
                if not matching_rooms:
                    ...
                    write_json(
                        final_booking_log, f"{BOOKING_LOG_FILEPATH}scraped_log.json"
                    )
                    ...
                    room_names_array_raw = [
                        room.inner_text()
                        for room in frame.query_selector_all(
                    ...
                    bookings_array_raw = [
                        active_bookings.get_attribute("title")
                        for active_bookings in frame.query_selector_all(
                    ...
                    for index, booking_array in enumerate(bookings_array_sanitised):
                        ...
                        for booking in booking_array:
                            ...
                            if booking.startswith("Booking Time:"):  # existing booking
                                ...
                                # edge case checking
                                print(
                                    f"Unrecognised timeslot format, logged here: {booking}"
                                )
                        ...
                        room_timeslot_map[room_names_array_sanitised[index]] = (
                            fill_missing_timeslots(booking_details)
                        )
